...
default_hooks = dict(
    checkpoint=dict(interval=30, type='CheckpointHook'),
...
model = dict(backbone=dict(depth=53,init_cfg=dict(checkpoint='open-mmlab://darknet53', type='Pretrained'),out_indices=(3,4,5,),type='Darknet'),
             bbox_head=dict(anchor_generator=dict(base_sizes=
             [
...
test_dataloader = dict(
    batch_size=2,
    dataset=dict(
        ann_file='/root/autodl-tmp/VOCdevkit/VOC2007/ImageSets/Main/test.txt',
        backend_args=None,
...
train_dataloader = dict(
    ...
    dataset=dict(
        ann_file=
        '/root/autodl-tmp/VOCdevkit/VOC2007/ImageSets/Main/trainval.txt',
        backend_args=None,
        data_prefix=dict(sub_data_root='VOC2007/'),
        data_root='/root/autodl-tmp/VOCdevkit/',
        ...
        pipeline=[
            dict(backend_args=None, type='LoadImageFromFile'),
            dict(type='LoadAnnotations', with_bbox=True),
            dict(mean=[0,0,0,],ratio_range=(1,2,),to_rgb=True,type='Expand'),
            dict(min_crop_size=0.3,min_ious=(0.4,0.5,0.6,0.7,0.8,0.9,),type='MinIoURandomCrop'),
            dict(keep_ratio=True,scale=[(320,320,),(608,608,),],type='RandomResize'),
...
work_dir = '../autodl-fs/work_dir/custom_yolov3'
